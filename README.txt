...
- For making this model to inference one need to install all requirements which are listed in requirement.txt and trained weightes located in weights directory. 
- There is an inference function in main.py module which simply take two arguements, positional and keyword argument. Positional arguement is the path to screenshot webpages and keyword arguements specify the type of device on which one is going to use the model, it sets on "0" which indicate the model is run on GPU by default. 
